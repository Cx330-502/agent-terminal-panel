...
- 最近关闭恢复会保留自动名称属性；原编号空闲时继续使用，已被新会话占用时自动选择下一个空位，避免生成重复的自动标签。
- 会话新增明确的 `default`、`profile`、`custom`、`historyResume`、`historyFork` 启动来源，圆形箭头统一表示“重新运行启动命令”，并根据来源准确提示将执行当前默认命令、保存命令、一次性命令或 Provider Resume。
- Fork 启动继续禁止重复执行；默认命令会在重新运行时读取最新设置，Profile、自定义与历史 Resume 会重跑该标签保存的命令。
- 新增键值式 `agentTerminalPanel.launchCommands` 设置，可在 VS Code 设置页像环境变量一样逐项添加名称和命令，无需手写对象数组。
- 旧 `launchProfiles` 保持完整读取兼容并标记为 deprecated；新旧配置同名时由 `launchCommands` 优先，设置修改仍会即时刷新加号旁菜单。
- 新增自动编号、键值命令、旧配置合并优先级和启动来源提示回归。
...
- 主 `+` 继续直接运行 `launchCommand`；旁边箭头改为在按钮下方展开 Webview 内菜单，不再打开全局 Quick Pick。
- 启动配置显示名称与完整命令，点击后用配置名称创建终端；配置修改可热刷新，且不会改变默认命令或混入上次窗口默认会话恢复。
- 下拉菜单保留选择 cwd、一次性自定义命令、Provider 历史、窗口恢复和设置入口，支持左右会话栏、窄视图、键盘导航、Esc 与点击外部关闭。
- 移除视图标题栏中重复的下拉按钮，新增启动配置解析和四视口锚定菜单回归，并纳入完整九视口 UI 检查。

## 0.7.2

- 修复 Agent 持续输出时用户上拉查看历史内容会被异步写入回调反复强制滚回底部的问题。
- 终端现在仅在视口原本位于底部时跟随新输出；用户主动上拉后保持当前位置，重新滚到底部后自动恢复跟随。
- 新增高频排队输出与持续输出浏览器回归，覆盖上拉锁定、输出完成后保持和回到底部后恢复跟随。

...
- **不是只在作者机器能跑**：每次 PR 和主分支更新都会在 Ubuntu、Windows、macOS 上运行真实 PTY 测试，并执行 10 组 Chromium 交互回归后再允许发布。
...
4. `+` 旁边的箭头会就地展开启动菜单，优先显示自定义启动配置，并保留选择 cwd、一次性自定义命令和 Provider 历史会话。
...
可以用 `launchProfiles` 保存多个常用入口。主 `+` 仍执行默认命令，配置项只出现在旁边的下拉菜单中：
...
数组顺序就是菜单顺序，`name` 同时作为新终端的初始名称。配置修改会立即刷新菜单，不需要重载窗口。
...
- 只有默认 `+` 创建并能关联到内置 Codex / Claude Session ID 的会话及其后续恢复会话会被记录；启动配置、一次性自定义命令和手工 Provider 历史启动不会记录。
...
- 可重启会话关闭后会在内存中保留 30 分钟，最多保留最近 10 个；不会写入 workspaceState，也不会跨窗口重启保留。
...
| `agentTerminalPanel.launchProfiles` | `[]` | 加号旁下拉菜单中的有序名称/命令列表 |
...
Marketplace 会按当前 extension host 自动安装对应目标。GitHub `releases/v0.9.1/` 同时提供：

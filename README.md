...
4. `+` 旁边的下拉箭头集中提供选择 cwd、一次性自定义命令和 Provider 历史会话。
...
- 只有默认 `+` 创建并能关联到内置 Codex / Claude Session ID 的会话及其后续恢复会话会被记录；下拉菜单中的自定义命令和手工 Provider 历史启动不会记录。
...
Marketplace 会按当前 extension host 自动安装对应目标。GitHub `releases/v0.7.2/` 同时提供：
